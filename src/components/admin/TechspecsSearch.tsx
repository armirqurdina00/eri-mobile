...
import type { TechspecsPhone } from "@/app/api/techspecs/search/route";

interface TechspecsSearchProps {
  onSelect: (phone: TechspecsPhone) => void;
}

export default function TechspecsSearch({ onSelect }: TechspecsSearchProps) {
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<TechspecsPhone[]>([]);
  ...
  const search = useCallback(async (q: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/techspecs/search?q=${encodeURIComponent(q)}`);
  ...
  function handleSelect(phone: TechspecsPhone) {
    setSelected(phone.name);
    setQuery("");
    setOpen(false);
    setResults([]);
    onSelect(phone);
  ...
  return (
    <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-900 dark:bg-blue-950/30">
      ...
      <p className="mb-3 text-xs text-blue-700 dark:text-blue-400">
        Search a device to auto-populate specs, colors, and storage options.
      ...
      <div ref={containerRef} className="relative">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search e.g. iPhone 16 Pro…"
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400 truncate">
                    {phone.colors.slice(0, 3).join(", ")}
                    {phone.colors.length > 3 && ` +${phone.colors.length - 3} more`}
                    {" · "}
                ...
                <span className="shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-500 dark:bg-gray-800 dark:text-gray-400">
                  {phone.releaseDate ? phone.releaseDate.slice(0, 4) : "—"}
                </span>
              </button>
            ))}
          </div>
